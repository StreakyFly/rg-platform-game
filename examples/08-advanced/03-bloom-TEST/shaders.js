...
const magicVertex = `#version 300 es
layout (location = 0) in vec3 aPosition;
layout (location = 1) in vec2 aTexCoord;
layout (location = 2) in vec3 aNormal;
...
const magicFragment = `#version 300 es
precision mediump float;
precision mediump sampler2D;
...
// Textures and factors for bloom from renderGeometryBufferFragment
uniform sampler2D uEmissionTexture;
uniform float uEmissionStrength;
uniform float uExposure;
...
void main() {
    ...
    // Bloom
    vec3 diffuseBloom = pow(texture(uBaseTexture, vTexCoord).rgb, vec3(2.2));
    vec3 emission = pow(texture(uEmissionTexture, vTexCoord).rgb, vec3(2.2));
    vec3 colorBloom = diffuseBloom + uEmissionStrength * emission;
    ...
    // oColor = vec4(colorBloom * uExposure, 1);  // Bloom color
    // oColor = vec4(linearTosRGB(finalColor), 1);  // Light color
    // oColor = vec4(finalColor, 1);  // Light color fixed for Bloom.

    // vec3 combinedColor = linearTosRGB(finalColor) + colorBloom * uExposure;
    
    // vec3 combinedColor = finalColor * colorBloom * uExposure;
    vec3 combinedColor = finalColor + colorBloom * uExposure;
    oColor = vec4(combinedColor, 1);
}
`;
